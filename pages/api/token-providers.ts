...
export default async function tokenProvidersHandler(
  ...
  try {
    const { data, headers } = await axios.get( (process.env.API_ROOT || 'http://localhost:3040/') + 'token-providers', {
      headers: {
        auth, token
      }
    });

    console.log('tokenProviderHandler--- data retrieved:', data, 'headers are ', headers);
